...
const getResults = async (req, res)=>{
    try {
        ...
        ? fetchResult.filter(obj => obj.matricNo.toString() === `${searchQuery}`)
        // ? fetchResult.filter(obj => obj.matricNo.includes(searchQuery))
        : []
        // res.json(filteredResult)

        const semesterOne = filteredResult.filter(obj => obj.registeredCourse.semester === "one")
        const semesterTwo = filteredResult.filter(obj => obj.registeredCourse.semester === "two")
        const semesterThree = filteredResult.filter(obj => obj.registeredCourse.semester === "three")
        const semesterFour = filteredResult.filter(obj => obj.registeredCourse.semester === "four")
        const semesterFive = filteredResult.filter(obj => obj.registeredCourse.semester === "five")
        const semesterSix = filteredResult.filter(obj => obj.registeredCourse.semester === "six")
