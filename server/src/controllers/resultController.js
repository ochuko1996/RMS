...
const getResult = async (req, res) => {
    ...
    try {
        ...
        semesters.forEach(semester => {
            const semesterData = fetchResult.filter(obj => obj.registeredCourse.semester === semester);
            const points = semesterData.map(data => data.point);
            const courseUnits = semesterData.map(obj => obj.registeredCourse.course.unit);
            const GPA = gpa(points, courseUnits).toFixed(2);

            result.push({
                [`semester${semester.replace(/^\w/, c => c.toUpperCase())}`]: semesterData,
...
const getResults = async (req, res)=>{
    try {
        ...
        semesters.forEach(semester => {
            const semesterData = filteredResult.filter(obj => obj.registeredCourse.semester === semester)
            const points = semesterData.map(data => data.point)
            const courseUnits = semesterData.map(obj => obj.registeredCourse.course.unit)
            const GPA = gpa(points, courseUnits).toFixed(2)
